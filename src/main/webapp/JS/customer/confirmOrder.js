...
$(function() {
	...
	$("#orderConfirmationLI").addClass('menu-item-here');
	...
	$('#custOrdDateRange').daterangepicker({
		autoclose: true,
		buttonClasses: ' btn',
		...
		$('#custOrdDateRange .form-control').val(start.format('DD-MMM-YYYY') + ' / ' + end.format('DD-MMM-YYYY'));
	});

	$('#custOrdDateRange').on('cancel.daterangepicker', function(ev, picker) {
	...
	$('#srchMain').keyup(function(event) {
			...
			quotationDataTable.ajax.url('confirmOrderDataTableFilter?qry=' + $("#srchMain").val().trim() +
				'&customerId=' + $('#customerId').val()).load();
	});
	...
	if ($('#cb_confirmOrderDataGrid').prop("checked") == true) {
		...
		for (var i = 0; i < quotationLineid.length; i++) {
			...
			if (parseInt(rowData.supplierQuoted) > 1) {
				...
				console.log("Tete"+map[quotationLineid[i]].rows);
...
//				console.log("#confirmOrderDataGrid_" + quotationLineid[i] + "_table");
