	...
	$("#registredAddress").change(function(){
		if($(this).prop("checked")){
            ...
            $("#searchedDeliveryAddresses").on('change',function(e){
                ...
                $('#regBillingTown').val(address.post_town);
...
function saveRegistration() {
	...
	if($('#gphcFileDZ')[0].dropzone.files[0] === undefined){
		toastr.warning("Please upload GPHC license");
		return false;
	}
	if($('#gphcFileDZ')[0].dropzone.files[0].type !== "application/pdf"){
		toastr.warning("Please upload GPHC files only");
		return false;
	}
	var formSerializeArray = $('#customerRegistrationForm').serializeArray();
	formSerializeArray.push( {name:'licenseFile', value:$('#gphcFileDZ')[0].dropzone.files[0]});
	 var fd = new FormData();
	  $.each(formSerializeArray, function(i, field){
	    fd.append(field.name , field.value);
	  });
//	var formSerializeArray = $('#customerRegistrationForm')[0];
//	 var fd = new FormData(formSerializeArray);
	  swal.fire({
			    title: "Are you sure?",
