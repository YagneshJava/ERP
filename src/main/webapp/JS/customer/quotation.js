...
$(function() {
	...
	$("#customerTenderPageLI").addClass('menu-item-here');
	$(".widget ul li").prepend("<i class=\"fas fa-circle mr-1\"></i>");
	...
	if (localStorage.getItem("divFlag") == "true") {
		...
		getQuotationData(localStorage.getItem("quotationId"));
	} else {
...
getQuotationList();
	...
	$('#custQtDateRange').daterangepicker({
		autoclose: true,
		buttonClasses: ' btn',
		...
		locale: {
            ...
            $('#custQtDateRange .form-control').val( start.format('DD-MMM-YYYY') + ' / ' + end.format('DD-MMM-YYYY'));
        });
        
        $('#custQtDateRange').on('cancel.daterangepicker', function(ev, picker) {
		$('#srchStartEndDate').val('');
	});
	
	$('#mainSrch').keyup(function(event) {
		
			var quotationDataTable = $('#quotationDataTable').DataTable();
			quotationDataTable.ajax.url('quotationDataTableFilter?qry=' + $("#mainSrch").val().trim() +
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.lines != null && data.lines.length > 0 && data.head != undefined) {
				...
				localStorage.setItem("quotationId", data.head.quotationId);
...
function loadQuotationGrid(quotationId) {
	...
	$("#quotationGrid").jqGrid({
		...
		colModel: [
			...
			{ name: 'description', index: 'description', frozen: true, width: '250' },
			{
...
//	$("#quotationGrid").jqGrid('clearGridData');
	$.ajax({
		url: "addLine",
		data: {
