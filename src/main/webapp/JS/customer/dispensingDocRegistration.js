...
function saveRegistration() {
	if($('#cqcFileDZ')[0].dropzone.files[0] === undefined){
		toastr.warning("Please upload CQC license");
		return false;
	}
	if($('#cqcFileDZ')[0].dropzone.files[0].type !== "application/pdf"){
		toastr.warning("Please upload PDF files only");
		return false;
	}
	
	alert( $("#country").val()+" "+$("#city").val()+" "+$("#county").val());
	var formSerializeArray = $('#customerRegistrationForm').serializeArray();
	formSerializeArray.push( {name:'licenseFile', value:$('#cqcFileDZ')[0].dropzone.files[0]});
	 var fd = new FormData();
//	  fd.append("Pratik" , "Student");
//	  fd.append("Yagnesh" , "Emplyee");
//	  fd.append("Sandeep" , "Senior");
