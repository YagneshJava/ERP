...
$(function() {
	...
	$("#manufacturerOrderConfirmationLI").addClass('menu-item-here');
	...
	if (localStorage.getItem("supplierConfirmOrderDivFlag") == "true") {
		...
		$('#kt_sidebar').show();
		getQuotationData(localStorage.getItem("supplierOrderQuotationId"), localStorage.getItem("supplierOrderCustomerId")	,localStorage.getItem("supplierOrderId"));
	} else {
	...
	$("#supplierConfirmOrderGrid").jqGrid({
		...
		url : 'getUniqueQuotationForSupplier?supplierId='+$('#manufacturerId').val(),
		mtype: "POST",
		datatype: "json",
		rowNum: 100,
		rowList: [100, 200, 500],
		colNames: ['Quotation ID', 'Order ID','Customer ID','Manufacturer ID', 'Customer Name','Order ID','Status', 'Order Date', 'Action'],
		colModel: [
			{ name: 'quotationId', index: 'quotationId',align: "left" },
			...
			{ name: 'supplierName', index: 'supplierName' ,align: "left"},
			{ name: 'orderId', index: 'orderId',hidden:true },
			{ name: 'status', index: 'status',hidden:true },
			{ name: 'createdDate', index: 'createdDate' ,align: "left"},
...
//		shrinkToFit: false,
		...
		ondblClickRow: function(rowId) {
			...
			getQuotationData(quotationId, customerId, orderId)
		},
	});
...
$('#manuOrdSrchDateRange').daterangepicker({
		 autoclose: true,
		 buttonClasses: ' btn',
		 ...
		 locale: {
            ...
            $('#manuOrdSrchDateRange .form-control').val( start.format('DD-MMM-YYYY') + ' / ' + end.format('DD-MMM-YYYY'));
            
        });
	$('#manuOrdSrchDateRange').on('cancel.daterangepicker', function(ev, picker) {
...
function getQuotationData(quotationId, customerId, orderId) {
...
function loadConfirmOrderData(quotationId) {
	$("#supplierConfirmOrderDataGrid").jqGrid({
		url: "getOrderLinesForSupplier?quotationId=" + quotationId + "&supplierId="+$('#manufacturerId').val(),
		mtype: "POST",
		datatype: "json",
		rowNum: 100,
		rowList: [100, 200, 500],
		colNames: ['Line No', 'Order Id', 'Pip Code', 'Product',  'Ordered Qty', 'Quotation Status', 'Ordered Price'],
		colModel: [
