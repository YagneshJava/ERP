...
$(document).ready(function() {
	...
	$("#supplierTenderPageLI").addClass('menu-item-here');
...
//	});
	 ...
	 $('#searchDateRangePickerSupplierQt').daterangepicker({
		 autoclose: true,
		 buttonClasses: ' btn',
		 ...
		 locale: {
            ...
            $('#searchDateRangePickerSupplierQt .form-control').val( start.format('DD-MMM-YYYY') + ' / ' + end.format('DD-MMM-YYYY'));
            
        });
	$('#searchDateRangePickerSupplierQt').on('cancel.daterangepicker', function(ev, picker) {
		...
		success: function(data) {
			...
			if (data.success != undefined) {
				...
				if (data.success === "Incorrect") {
					...
					jQuery("#quotationGrid").jqGrid('bindKeys');
//					jQuery("#quotationGrid").jqGrid('bindKeys', {
//						"onEnter": function(rowid) {
//							console.log("You enter a row with id:" + rowid)
//						}
//					});
//					jQuery("#quotationGrid").jqGrid('bindKeys');
...
function loadQuotationLineGrid(quotationId, addressId) {
	...
	$("#quotationGrid").jqGrid({
		url: "qoutationLineForSupplier?quotationId=" + quotationId + "&supplierId=" + $('#supplierId').val()+"&addressId="+addressId,
		...
		colModel: [
			...
			{
				name: 'supplierQuotedQty', index: 'supplierQuotedQty', editable: true, editoptions: {
					...
					dataEvents: [
		      		            {
		    					    ...
		    					    fn: function(e) {
										...
										if (key == 13) {
											...
											jQuery('#quotationGrid').jqGrid('setSelection', parseInt(col1) + 1, false);
		    					        }
		    					    }
		    					}
		          		       ]
			...
			{
				name: 'quotedPrice', index: 'quotedPrice', editable: true, align:'center' ,formatter: blankFormatter, editoptions: {
					dataInit: function(element) {
						$(element).keypress(function(e) {
			...
			{ name: 'sellOverview', index: 'sellOverview', editable: false, formatter: sellOverviewFormatter },
			{ name: 'brandName', index: 'brandName', editable : true, editoptions:{
				dataEvents: [
		      		            {
		    					    type: 'keydown',
		    					    fn: function(e) {
			...
			{ name: 'leadTime', index: 'leadTime', editable : true, align:'center',editoptions:{
				dataEvents: [
		      		            {
		    					    type: 'keydown',
		    					    fn: function(e) {
...
//		cellurl: 'updateSupplierLineData',
	...
	$('#quotationGrid').jqGrid('setLabel', 'expiryDate', '', { 'text-align': 'center' });
	...
	$('#quotationGrid').jqGrid('setLabel', 'leadTime', '', { 'text-align': 'center' });
}
...
function lineEditFormatter(cellvalue, options, rowObject) {
	if ($('#savedStatus').val() == "Submitted" || $('#savedStatus').val() == "PartiallyConfirmed") {
		return "<a onclick='editQuotationRow(\"" + rowObject.supplierQuotationLineId + "\");' title='Edit' href='javascript:void(0)'title='Edit' href='javascript:void(0)'class='btn btn-sm btn-bg-white btn-clean btn-icon-warning  btn-hover-warning  btn-icon mr-2 my-1 my-lg-0' > <i class='la la-pencil'></i></a>";
	} else {
...
//	if(ids.length<grid.getGridParam("reccount")){
//		swal.fire({
//			title: "Are you sure?",
//			text: "You don't want to confirm all Lines!",
//			icon: 'warning',
...
console.log($('#supplierLocation').val());
...
function sellOverviewFormatter(cellvalue, options, rowObject) {
	if ($('#savedStatus').val() == "Submitted" || $('#savedStatus').val() == "PartiallyConfirmed") {
		return "<a onclick='getSellOverview(\"" + rowObject.pipCode + "\");' title='Potential' href='javascript:void(0)'class='btn btn-sm btn-bg-white btn-clean btn-icon-info  btn-hover-info  btn-icon mr-2 my-1 my-lg-0' ><i class='fa la-info text-info'></i> </a>" ;
	} else {
		return "<a title='Edit' href='javascript:void(0)'title='Potential' href='javascript:void(0)'class='btn btn-sm btn-bg-white btn-clean btn-icon-success  btn-hover-success  btn-icon mr-2 my-1 my-lg-0' ><i class='fa la-info text-success'></i> </a>" ;
	}
}
