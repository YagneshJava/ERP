...
//	}
	...
	if($('#wdaFileDZ')[0].dropzone.files[0] === undefined){
		toastr.warning("Please upload WDA license");
		return false;
	}
	if($('#wdaFileDZ')[0].dropzone.files[0].type !== "application/pdf"){
		toastr.warning("Please upload PDF files only");
		return false;
	}
	
	var formSerializeArray = $('#customerRegistrationForm').serializeArray();
	if($('#kt_dropzone_1')[0].dropzone.files[0] !== undefined && $('#kt_dropzone_1')[0].dropzone.files[0].type === "application/pdf"){
		formSerializeArray.push( {name:'licenseFile', value:$('#kt_dropzone_1')[0].dropzone.files[0]});
	}
	
	formSerializeArray.push( {name:'wdaLicenseFile', value:$('#wdaFileDZ')[0].dropzone.files[0]});
